...
import { isFeishuRow } from '../data/row-guards'
import { type FeishuQueryResponse } from '../data/schema'
import { FeishuConfigDialog } from './feishu-config-dialog'
import { FeishuDeleteDialog } from './feishu-delete-dialog'
...
export function FeishuNotificationsPanel() {
  ...
  const queryParams = {
    ...
    feishu_name: filters.feishu_name.trim(),
    feishu_url: filters.feishu_url.trim(),
  ...
  const { data, isLoading, isError, isFetching } = useQuery({
    queryKey: ['/api/notifications/notify_feishu_info', queryParams],
    queryFn: async () => {
      const response = await api.get<FeishuQueryResponse>(
  ...
  const feishus = data?.data.metadata ?? []
  const pagination = data?.data.pagination
  const totalRecords = pagination?.total ?? feishus.length
  ...
  if (isError) {
    return (
      <Alert variant='destructive'>
        <AlertCircle />
        <AlertTitle>飞书配置加载失败</AlertTitle>
        <AlertDescription>
          请稍后重试，或检查通知服务接口是否可用。
        </AlertDescription>
      </Alert>
  ...
  return (
    <div className='space-y-4'>
        ...
        <CardContent className='space-y-4'>
          <div className='rounded-xl border bg-muted/20 p-4 sm:p-5'>
            <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
              ...
              <div className='space-y-2'>
                <label className='text-sm font-medium'>飞书名称</label>
                <Input
                  value={draftFilters.feishu_name}
                  placeholder='按名称模糊匹配搜索'
                  onChange={(event) =>
                    setDraftFilters((prev) => ({
                      ...prev,
                      feishu_name: event.target.value,
                    }))
                  }
                />
              </div>
              <div className='space-y-2'>
                <label className='text-sm font-medium'>飞书链接</label>
                <Input
                  value={draftFilters.feishu_url}
                  placeholder='按飞书链接模糊匹配搜索'
                  onChange={(event) =>
                    setDraftFilters((prev) => ({
                      ...prev,
                      feishu_url: event.target.value,
                    }))
                  }
                />
              </div>
      ...
      <Card>
        ...
        <CardContent className='space-y-4'>
          {isLoading ? (
            <p className='text-sm text-muted-foreground'>正在加载飞书配置...</p>
          ) : (
            <div className='space-y-4'>
